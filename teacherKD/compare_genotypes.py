...
def get_distinct_genotype(genotype_dir, names=None):
  distinct_genotype = []
  distinct_names = []
  if names is None:
      names = range(len(os.listdir(genotype_dir)))
  for name in names:
      genotype_file = os.path.join(genotype_dir, '%s.txt'%name)
...
if __name__ == '__main__':
  base_dir = 'search-EXP-20200517-160203/'
