...
parser.add_argument('--train_weight_portion', type=float, default=0.8,
                    help='portion of training weight, use only when multisetp is TRUE')
parser.add_argument('--accumulate_gradient', action='store_true', default=False)
parser.add_argument('--accumulate_step', type=int, default=10)
...
def train(train_queue, valid_queue, model, architect, optimizer, lr):
    ...
    for step, (input, target) in enumerate(train_queue):
      ...
      loss.backward()
      if (not args.accumulate_gradient) or (args.accumulate_gradient and (step + 1) % args.accumulate_step == 0):
