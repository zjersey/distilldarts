...
#parser.add_argument('--multistep', action='store_true', default=False, help='trains alpha and weight each for multiple steps')
#parser.add_argument('--train_weight_portion', type=float, default=0.8, help='portion of training weight, use only when multisetp is TRUE')
...
def main():
  ...
  model = Network(args.init_channels, CLASSES, args.layers, criterion, primitives, efficient=args.efficient ,drop_path_prob=args.drop_path_prob)
  ...
  train_queue = torch.utils.data.DataLoader(
      ...
      pin_memory=True, num_workers=0)  #length:391
