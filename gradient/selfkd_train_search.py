...
parser.add_argument('--T', type=float, default=3., help='temparature of KD')
parser.add_argument('--kd_alpha', type=float, default=0.7, help='weight of KD_logits_Loss')
parser.add_argument('--kd_beta', type=float, default=0.05, help='weight of KD_featureMap_Loss')
...
def main():
  ...
  if not args.test:
    ...
    writer = csv.writer(f)
    title = ['epoch', 'train_acc', 'train_loss',
             'train_celoss', 'train_klloss', 'train_fmloss'] + ['train_acc%d' % layer_num for layer_num in range(7)] \
            + ['gradient_layer%d'% i for i in range(args.layers)]
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    # training
    train_acc_set, train_loss_set, train_gradients = train(train_queue, valid_queue, model, architect, optimizer, lr, epoch, gradients_file)
    train_acc = train_acc_set[0]

    logging.info('train_acc %f', train_acc)
    for i in range(args.layers):
      writers_total[i].add_scalar('avg_gradient-selfkd200', train_gradients[i], epoch)
    # validation
    ...
    with open(result_file, 'a') as f:
      writer = csv.writer(f)
      a = [epoch, train_acc] + train_loss_set + train_acc_set[1] + train_gradients
      writer.writerow(a)
...
def train(train_queue, valid_queue, model, architect, optimizer, lr, epoch, gradients_file):
  ...
  for step, (input, target) in enumerate(train_queue):
    ...
    [loss, CEloss, KLloss, FMloss], logits_set, outputs= model.compute_loss(input, target, return_logits=True)
    logits = logits_set[-1]
    ...
    for i, layer in enumerate(outputs):
      ...
      writers_oneEpoch[i].add_scalar('epoch-%d-selfkd200' % epoch, norm_gradient, step)
  ...
  f.close()
  return [top1.avg, top1_auxiliaries_avg], [objs.avg, celoss.avg, klloss.avg, fmloss.avg], gradients_avg
