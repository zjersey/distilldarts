...
class Architect(object):
  ...
  def _backward_step(self, hidden, input, target):
    loss, hidden_next = self.model._loss(hidden, input, target)
    if self.reg_alpha > 0.:
      alpha = self.model.get_softmax_arch_parameters()[0]
      prune_mask = self.model.prune_mask
      reg_loss = 0.
      if self.reg_type == 'norm':
        reg_loss += (alpha*(1. - prune_mask.data.float())).pow(2).sum()
      elif self.reg_type == 'gini':
        tmp = alpha * (1. - prune_mask.float())
        reg_loss += (tmp * (1.-tmp)).sum()
      else:
        raise(NotImplementedError("Not implement regularization type as"%self.reg_type))
  ...
  def _backward_step_unrolled(self,
    ...
    # regularization
    if self.reg_alpha > 0.:
      alpha = self.model.arch_parameters()[0]
      mask = self.model.prune_mask
      alpha.grad.data.add_(self.reg_alpha*2.*(1.-mask.data.float())*alpha.data)
